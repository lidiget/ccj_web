<template>
  <a-card style="margin-top: 24px" :bordered="false">
    ...
    <a-button
      ...
      @click="daochuliebiao"
      :loading="spinning"
      v-if="$store.state.rights[12].children[1].children[0].import == 1"
    >
      导出列表
    </a-button>
    ...
    daochuliebiao() {
      ...
      this.$confirm({
        ...
        onOk: () => {
          ...
          this.$request({
            url: '/user/getAllSalesRankingAllData',
            method: 'get',
            params: formdate,
          })
            .then((res) => {
              if (res.code == 200) {
                ...
                this.outputXlsxFile(alldata, '材料销量排行列表')
              }
            })
            .catch(function () {
              that.spinning = false
