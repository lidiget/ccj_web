<template>
  <a-card style="margin-top: 24px" :bordered="false">
    <a-row :gutter="8">
        ...
        <div>
          ...
          <a-card>
            ...
            <a-tree
              ...
              "
            />
            <div :style="this.tmpStyle" v-if="NodeTreeItem && $store.state.rights[4].children[0].children[0].edit == 1">
              <div
                class="menu-item"
                @click="orgAdd"
                style="margin-left: 8px"
      ...
      <a-col :span="19">
        ...
        <a-button
          ...
          @click="addGooddata"
          :loading="spinning"
          v-if="$store.state.rights[4].children[0].children[0].add == 1"
        >
          +新增材料
        </a-button>
        <a-button
          ...
          "
          :loading="spinning"
          v-if="$store.state.rights[4].children[0].children[0].upload == 1"
        >
          批量导入
        </a-button>
        <a-button
        ...
        <a-button
          ...
          @click="daochuliebiao"
          :loading="spinning"
          v-if="$store.state.rights[4].children[0].children[0].import == 1"
        >
          导出材料信息
        </a-button>
        <a-spin :spinning="spinning" size="large">
          ...
          >
            ...
            <span slot="action" slot-scope="text, record">
              <template v-if="$store.state.rights[4].children[0].children[0].edit == 1">
    ...
    <div>
      <a-modal
        :title="ifaddgoodsclass ? '新增材料分类' : '编辑材料分类'"
  ...
  methods: {
    ...
    handlegoodsclass() {
      ...
      if (that.ifaddgoodsclass) {
        ...
        formdate.shop_goods_ifzhixiao = 0
    ...
    dealFile: function (data) {
      ...
      if (data.length <= 0) {
        ...
        function adds() {
          ...
          if (tmp.length > 0) {
            tmp.forEach((item) => {
              var data = { error: '抱歉,表格中材料代码：' + item + ',多个材料填写重复,请勿填写重复代码' }
              that.errordetail.push(data)
            })
            that.spinning = false
            that.failecailiaodaoru = true
    ...
    asss() {
      ...
      if (that.ifaddcommomuserdata) {
        ...
        console.log(this.AllGoodsSize)
        // var haveno = false
        // this.AllGoodsSize.forEach((item) => {
        //   if (item.guigedaima == '' || item.guigedaima == null) {
        //     haveno = true
        //   }
        //   if (item.guigename == '' || item.guigename == null) {
        //     haveno = true
        //   }
        // })
        ...
        // console.log(haveno)
        // if (haveno) {
        //   that.$message.warning('抱歉,规格名称和规格代码不能为空!')
        //   return
        // }
